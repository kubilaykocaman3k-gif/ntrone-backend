...
router.post('/',    async (req,res) => { try { const d=await Order.create({req.body}); res.status(201).json({success:true,data:d}); } catch(e){res.status(400).json({success:false,message:e.message});} });
